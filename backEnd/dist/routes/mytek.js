...
const mytek_1 = __importDefault(require("../models/mytek"));
...
router.get(`/item/:id`, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const item = yield mytek_1.default.findById(req.params.id);
        console.log(item);
        if (!item) {
            res.status(404).json({ success: false, message: 'mytek not found.' });
        }
        else {
            res.send(item);
        }
