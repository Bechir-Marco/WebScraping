...
class PetitelectrocuisineSpider(scrapy.Spider):
    name = "petitElectroCuisine"
    ...
    def start_requests(self):
        for pageNum in range(1, 48):
            url = f"https://www.tunisianet.com.tn/522-petit-electromenager-tunisie-cuisine?page={pageNum}&order=product.price.asc"
    ...
    def parse_book_page(self, response):
        ...
        extracted_text = extracted_text = response.xpath('//*[@id="wrapper"]/div/div/nav/ol/li[4]/a/span/text()').get()
        book_item["category"] = extracted_text

        description = response.css(".product-information p::text").extract()
        if not description:
            description = response.xpath('string(//*[starts-with(@id, "product-description-short-")])').get()
        ...
        book_item["image"] = response.css('meta[property="og:image"]::attr(content)').get()
        yield book_item
